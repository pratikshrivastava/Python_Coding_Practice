...
def is_bst(node, lower_lim=None, upper_lim=None):
    ...
    if not (lower_lim is None) and (node.value < lower_lim):
        return False
    if not (upper_lim is None) and( node.value > upper_lim):
        return False
    ...
    if node.right and is_left_bst:
        is_right_bst = is_bst(node.right, node.value,upper_lim)

    print(node.value, lower_lim, upper_lim)
